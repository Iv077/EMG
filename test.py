...
classifier = joblib.load('EMG_Classifier.sav')          #   Calling saved dataset
  ...
  def record_data(self):
    if self.record:
      ...
      if  dif_time < self.record_time:
        ...
        sensor_data = []
        for i in range (8):
          sensor_data.append(self.emg_data[0][i]) # pd.concat????
          arr = np.array(self.emg_data[0][i])
          print(classifier.predict(arr.reshape(2772,1,-1))) #-----------------------------------------------------------------------------------------------------------------------------------------

        try:
          self.record_file.write(sensor_data)
        except Exception:
          pass

        print(arr.reshape(2772,1,-1))
